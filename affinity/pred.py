...
def tet_result(model, criterion, test_loader, device):
    criterion = nn.MSELoss()
    test_loss, test_cindex, test_r2 = tet_val(model, criterion, test_loader, device)
    return test_loss, test_cindex, test_r2


def tet_val(model, criterion, dataloader, device):
    model.eval()
    running_loss = AverageMeter()
    ...
    for i in range(len(pred)):
        print("data_{}     pred:{}    label:{}".format(i, pred[i], label[i]))
...
def main():
    ...
    parser.add_argument('--dataset', default='davis_0', help='task name')
    parser.add_argument('--save_model', action='store_true', help='whether save model or not')
    parser.add_argument('--lr', type=float, default=3e-4, help='learning rate')
    parser.add_argument('--batch_size', type=int, default=2, help='batch_size')
    args = parser.parse_args()
    params = dict(
        data_root="hybrid_smi",
        save_dir="save",
    ...
    model = ColdDTA()
    model.load_state_dict(torch.load("model_affinity/epoch728test_loss0.1186.pt"))
    model.to("cuda")
    model.eval()

    criterion = nn.MSELoss()
    test_loader = DataLoader(test_set_list[4], batch_size=batch_size, shuffle=False, num_workers=num_workers,
                             drop_last=True)
    test_loss, test_cindex, test_r2 = tet_result(model, criterion, test_loader, device)
    return

...
#sample
    ...
    chunk_tensors = torch.chunk(sample, 100)
...
# Calculate similarity score
    ...
    for result in temp_results:
        ...
        if temp == 10:
            break
    start_smiles_sort = list(temp_unique_results.keys())
    scores_start_sort = list(temp_unique_results.values())

    apprentice_storage.add_list(smis=start_smiles_sort, scores=scores_start_sort)
    ...
    for step in tqdm(range(4)):
        ...
        gen_smiles = expert_handler.query(
            query_size=50, mating_pool=expert_smis, pool=pool
        )
        ...
        gen_scores = list(temp_unique_results.values())
    ...
    int_results = [OptResult(smiles=smiles, score=score) for smiles, score in
                   zip(canonicalized_smiles + total_smiles, scores_structure + total_scores)]
...
def OptimizeDDM(train_dataloader,total_epoch):
    ...
    opt = AdamW(
        mp_trainer.master_params, lr=0.0001, weight_decay=0.0
    )
...
if __name__ == "__main__":
     ...
     model.load_state_dict(th.load("op_diffusion/model_successful_1000.pth"))
     model.to("cuda")
     # model.eval()
     for param in model.parameters():  #
         param.requires_grad = False  #
     ...
     expert_handler = ga_operator(mutation_rate=0.05)

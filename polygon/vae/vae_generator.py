...
class SmilesVaeMoleculeGenerator:
    ...
    def optimise(self, objective: ScoringFunction, start_population, keep_top, n_epochs, mols_to_sample,
        ...
        objective：目标函数，用于评估分子的质量。
        start_population：初始分子群。    none
        keep_top：每次迭代优化中保留的分子数量。   #512
        n_epochs：优化的总迭代次数。   #20
        mols_to_sample：每次迭代中采样的分子数量。  #1024
        optimize_n_epochs：优化过程中每次微调的迭代次数。  #2
        optimize_batch_size：微调时的批量大小。  #256
        pretrain_n_epochs：在开始优化前预训练的迭代次数。   #2
        save_frequency：保存模型的频率，默认为每10次迭代保存一次。
        ind_scorers：用于单独评估分子的评分函数（可选）。
        save_payloads：是否保存中间结果（可选）。
        """
        ...
        for epoch in range(1, 1 + n_epochs):  #20
            ...
            samples = self.sample(mols_to_sample)   #1024
            t1 = time.time()
